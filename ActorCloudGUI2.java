...
class DrawPanel extends JPanel {
	...
	for (int i=0; i<actorList.size(); i++) {
	    ...
	    Color color = new Color (255-a.numMovies,255-(a.numMovies*2),255-a.numMovies);
	    ...
	    int x;
	    if (stringWidthInPixels >= D.width) {
		x = 0;
	    }
	    else {
		x = random (D.width - stringWidthInPixels);
	    }
	    
	    int y = random (D.height - stringHeight);
	    if (y < stringHeight) {
		y = stringHeight;
	    }
